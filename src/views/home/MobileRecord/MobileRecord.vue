    <template>
            <section id="MobileRecord">
                <van-nav-bar title="移动纪实" :fixed="true" left-arrow @click-left="back" :z-index="999">
                ...
                <van-tabs :sticky="true" :offset-top="46" v-model="active">
                    <van-tab title="全部">
                        <van-list v-model="all_loading" :finished="all_finished" finished-text="没有更多了" @load="onLoadAll">
                            <van-cell v-for="(item,index) in allList" :key="index">
                                <compMobileRecord :comp-data="item"></compMobileRecord>
                                <!-- 评论与赞-->
                    ...
                    <van-tab title="我的">
                        ...
                        <van-cell v-for="(item,index) in myList" :key="index">
                            ...
                            <!-- 评论与赞-->
                            <div class="active-box" v-if="item.status =='1'">
                                    <span class="comment-box" @click="comment(item)">
                                        <i class="icon iconfont">&#xe656;</i>
                                        <span class="comment">评论</span>
                                        <span class="comment-num">{{ (item.commentCount!=null)?item.commentCount:0 }}</span>
                        ...
                        <div class="creat-box" v-show="creatShow">
                            ...
                            <div class="dear">亲，你还没有移动纪实记录</div>
        ...
        <script>
            import compMobileRecord from '../MobileRecord/compMobileRecord';
            ...
            export default {
                ...
                mounted(){
                    ...
                    apiLists.dataAjax({}, 'MobileRecord_release', $id).then(res => {
                    })
                },
                methods:{
                    back(){
                                ...
                                if(res.code == 1001){
                                    ...
                                    for (let i = 0; i < data.length; i++) {
                                        me.allList.push({
                                            ...
                                            goPageUrl: '/MobileRecord/MobileDetails',
                                            id: data[i]._id,
                                        });
                                    }
                    ...
                    delet(parameter,flag){
                        this.$dialog.confirm({
                            ...
                            apiLists.dataAjax({}, 'MobileRecord_delet', parameter.id).then(res => {
                                console.log(res);
                                if (res.code == 1001) {
                                    setTimeout(() => {
                                        Toast.clear(50);
                    ...
                    release(parameter,flag){
                        this.$dialog.confirm({
                            ...
                            apiLists.dataAjax({}, 'MobileRecord_release', parameter.id).then(res => {
                                console.log(res);
                                if (res.code == 1001) {
                                    setTimeout(() => {
                                        Toast.clear(50);
                    ...
                    comment(parameter) {
                            this.$router.push({
                                path: '/MobileRecord/MobileComment',
                                query: {
                                    id:parameter.id,
                                }
                            });
